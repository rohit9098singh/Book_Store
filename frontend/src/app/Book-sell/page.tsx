...
const Page = () => {
  ...
  const onSubmit = useCallback(
    async (data: BookDetails) => {
      if (isSubmitting) return // Prevent multiple submissions
      setIsSubmitting(true)

      try {
        // Create FormData outside the main thread using a setTimeout
        // This helps to not block the main thread during form data preparation
        setTimeout(async () => {
          try {
    ...
    [addProduct, router, reset, isSubmitting],
  )
              ...
              <div className="flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-4">
                ...
                <div className="md:w-3/4 space-y-2">
                  <p className="text-sm text-neutral ">
                    After your book is sold in what mode would u like to recieve the payment?{" "}
                  </p>
                  <Controller
                    name="paymentMode"
                    control={control}
